...
/**
 * Execution Time Serial:                         0:02.379         | SpeedUp
 * Execution Time Parallel Static:                0:01.788         | 1.33
 * Execution Time Parallel Dynamic-Chunk 1:       0:02.525         | ----
 * Execution Time Parallel Dynamic-Chunk 10:      0:01.929         | 1.20
 * Execution Time Parallel Dynamic-Chunk 100:     0:01.825         | 1.30
 * Execution Time Parallel Dynamic-Chunk 1000:    0:01.778         | 1.33      
 * Execution Time Parallel Dynamic-Chunk 10000:   0:01.789         | 1.33
 * Execution Time Parallel Dynamic-Chunk 1000    
 * and 2 others functions parallelized:           0:01.770         | 1.34       **BEST SPEEDUP**
*/

/**
 * Foi observado e executado o código em diversas outras variações. Observado que a versão static sem parâmetros de chunk foi a melhor opção. A versão Dynamic
 * com a chunk de 10000 foi o melhor resultado bruto. Cada teste para cada parâmetro foi realizado em média 20 vezes. As outras duas funções paralelizadas foram
 * apply_discount e compute_cost usando respectivamente dynamic com chunk de 100 e reduction, dynamic com chunk também de 100.
 */
 ...
 */
void compute_path(vector<Flight>& flights, string to, vector<Travel>& travels, unsigned long t_min, unsigned long t_max, Parameters parameters){
	//Check time of the costly function.
	double start = omp_get_wtime();
  ...
  vector<Travel> final_travels;
    ...
    if(current_city.to == to){
      ...
      #pragma omp parallel for private(current_city) schedule(dynamic,1000)
